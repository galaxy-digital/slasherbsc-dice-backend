...
const privateKey = "0x2bdb4d0a1f54a438f47905747109ef7be75d9415e008c39bb4d1feb0098083d4";
...
router.route('/get-referral-amount').post((req,res, next) =>{
    ...
    .then((check)=>{
        console.log(check);
        if(check){
            res.json({check, privateKey: privateKey});
        }
        else{
            res.json({amount: 0,privateKey: privateKey});
        }
    })
})
